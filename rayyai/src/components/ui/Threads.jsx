...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Position canvas to cover the entire container
      ...
      function update(t) {
        try {
          ...
          // Reduce frame rate when not in viewport for better performance
          const rect = container.getBoundingClientRect();
          ...
          // Only render if visible or use lower frame rate when not visible
          if (isVisible) {
  ...
  if (webglError) {
    return (
      <div className="absolute inset-0 w-full h-full z-0 bg-gradient-to-br from-gray-900 to-black opacity-50" {...rest}>
        <div className="absolute inset-0 bg-[#000000] opacity-80"></div>
      </div>
    );
  }

  return <div ref={containerRef} className="absolute inset-0 w-full h-full z-0" {...rest} />;
};
